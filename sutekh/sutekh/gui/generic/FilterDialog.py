...
from ...core.generic import FilterParser
from .BaseConfigFile import PHYS_CARDLIST, CARDSET, DEF_PROFILE_FILTER
...
class FilterDialog(SutekhDialog):
    ...
    def __delete_filter(self):
        ...
        if sConfigFilter is not None:
            if dProfiles[PHYS_CARDLIST] or dProfiles[CARDSET]:
                # Filter is in use as a config filter, so prompt
                sCardlist = '\n'.join(['Cardlist profile : %s' %
                    self.__oConfig.get_profile_option(PHYS_CARDLIST, x, 'name')
                    for x in dProfiles[PHYS_CARDLIST]])
