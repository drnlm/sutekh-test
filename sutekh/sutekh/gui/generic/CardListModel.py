...
import gobject
import logging
from ...core.Filters import FilterAndBox, NullFilter, PhysicalCardFilter, \
        make_illegal_filter, CachedFilter
from ...core.Groupings import CardTypeGrouping
from ...core.Objects import (PhysicalCardToAbstractCardAdapter,
...
class CardListModel(gtk.TreeStore):
    ...
    def update_options(self, bSkipLoad=False):
        ...
        sProfile = self._oConfig.get_profile(PHYS_CARDLIST, PHYS_CARDLIST)
        sExpMode = self._oConfig.get_profile_option(PHYS_CARDLIST, sProfile,
                EXTRA_LEVEL_OPTION).lower()
        bExpMode = EXTRA_LEVEL_LOOKUP.get(sExpMode, True)

        bUseIcons = self._oConfig.get_profile_option(PHYS_CARDLIST, sProfile,
                USE_ICONS)

        bHideIllegal = self._oConfig.get_profile_option(PHYS_CARDLIST, sProfile,
                HIDE_ILLEGAL)

        sConfigFilter = self._oConfig.get_profile_option(PHYS_CARDLIST, sProfile,
                'filter')
