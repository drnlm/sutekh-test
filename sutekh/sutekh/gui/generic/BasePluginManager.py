...
"""Base Classes for managing and creating plugins."""
...
class BasePluginManager(object):
    ...
    def _do_load_plugins(self, sApp, mPlugins, cAllowedPlugin):
        """Load list of Plugin Classes from plugin dir."""
        for sPluginName in submodules(mPlugins):
            # load module
            # pylint: disable-msg=C0103
            # mPlugin is legal name here
            try:
                mPlugin = __import__("%s.gui.plugins.%s" % (sApp, sPluginName),
                        None, None, [mPlugins])
            ...
            if issubclass(cPlugin, cAllowedPlugin):
...
class PluginConfigFileListener(object):
    ...
    def profile_changed(self, sType, sId):
        ...
        if sType == FRAME and self._oPlugin.model.frame_id == sId:
            self._oPlugin.perpane_config_updated()
        elif sType in (CARDSET, PHYS_CARDLIST, CARDSET_LIST) and \
...
class BasePlugin(object):
    ...
    def get_perpane_item(self, sKey):
        ...
        if oModel is None or not hasattr(oModel, "cardset_id"):
            return None
        if oModel.cardset_id == PHYS_CARDLIST or \
